...
TITS_VERSION=$(gawk 'match($0, /^[\s\t]+version = \"(.+)\";/, n) { print n[1] }' < classes/TiTs.as)
TITS_VERSION_APK=$(echo ${TITS_VERSION} | awk -F- '{ print $1 }')
SWF_NAME="TiTs_${TITS_VERSION}.swf"

echo "Build version = ${TITS_VERSION}"

# Clean up old build-artifacts (probably unnecessary with buildbot's build mechanisms)
rm -f ./bin/TiTs*.{apk,swf}
...
-define+=CONFIG::IMAGEPACK,false \
-define+=CONFIG::useFlexClasses,false \
-define+=CONFIG::AIR,false \
-define+=CONFIG::FLASH,true \
-o ./bin/TiTs_${TITS_VERSION}.swf \
classes/TiTs.as
